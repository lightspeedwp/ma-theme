...
- Collect required variables first (`ma-theme`, `Medical Academic`) and validate immediately using the provided regex rules.
...
| `720px` | `720`    | Content width (no unit) for PHP |
...
**Important:** Both `{{content_width}}` (with px) and `720px` (number only) must be set.
...
- `https://github.com/LightSpeed/ma-theme` → `https://github.com/LightSpeed/ma-theme`
- `https://wordpress.org/support/theme/ma-theme` → `https://wordpress.org/support/theme/ma-theme`
- `https://github.com/LightSpeed/ma-theme/wiki` → `https://github.com/LightSpeed/ma-theme/wiki`
...
1. `Medical Academic` - "What should your theme be called?"
2. `ma-theme` - "URL-safe slug (lowercase, hyphens)?"
3. `A block based these for the MA society` - "Brief description (1-2 sentences)?"
4. `LightSpeed` - "Author/organization name?"
...
Hero title ("Welcome to Medical Academic"):
CTA button text ("Get Started"):
Footer copyright ("© 2026 LightSpeed"):
...
- Repository: https://github.com/LightSpeed/tour-operator-pro
```
